...
export default function CollegeClientView({ college, similarColleges }: { college: CollegeWithRelations; similarColleges: SimilarCollege[] }) {
  ...
  const extractImagesFromJsonb = (images: any): string[] => {
    ...
    return []; // 👈 Added fallback return if try/catch fails or Array.isArray is false
  };

  // ⚠️ Double check if it is image_url or image_urls in your DB!
  ...
  const getMainImage = (images: string[]): string => {
    ...
    return images[0]; // 👈 Added return statement and closed the function properly
  };

  const mainImageSrc = getMainImage(validImages);

  interface CollegeNews {
    id: number;
    category: string;
    ...
    news_data: {
      ...
      content: string;
      is_breaking?: boolean;
      image_urls?: any[]; // Replace with your JsonArray type if imported
    };
  }
    ...
    <div className="max-w-387 mx-auto bg-white min-h-screen m-0 p-0">
      ...
      <div className={`fixed bottom-0 w-full bg-white shadow-[0_-5px_8px_-1px_rgba(0,0,0,0.1)] z-50 transition-transform duration-500 ${isVisible ? 'translate-y-0' : 'translate-y-full'
        ...
        <div className="flex justify-center gap-4">
          ...
          <button
            ...
            className="bg-blue-600 text-white text-sm my-3 text-center px-4 py-2 rounded-lg font-medium hover:bg-blue-800 transition"
          >
            Apply Now
          </button>
          ...
          <button
            ...
            className="bg-white text-sm my-3 text-center border border-gray-400 px-4 py-2 rounded-lg font-medium hover:bg-blue-600 hover:text-white transition"
          >
            Explore ALl Course
          </button>
      ...
      <section className="bg-white px-3 md:px-6 lg:px-8">
        ...
        <div className="max-w-450 mx-auto flex flex-col-reverse lg:flex-row items-stretch">
          ...
          {/* RIGHT SIDE (IMAGE) */}
          <div className="w-full lg:w-2/4 bg-gray-50 lg:m-3 flex items-center justify-center overflow-hidden">
            <Image
              src={mainImageSrc}
              alt="Campus"
      ...
      {/* 2. STICKY NAV */}
      <nav className="max-w-387 mx-auto sticky top-17 z-30 bg-white border-y border-gray-300 overflow-x-auto no-scrollbar">
          ...
          {activeTab === 'news' && (
            <>
              ...
              <section className="bg-white p-4 lg:p-8 rounded-2xl shadow-sm" id="latest-news-updates">
                ...
                {/* <NewsTab news={college.news as unknown as Array<{ id: number; category: string; published_date: string | Date; news_data: { title: string; content: string; is_breaking?: boolean; image_urls?: any[] } }>} collegeName={college.name} /> */}
                <NewsTab
                  news={college.news as CollegeNews[]}
                  collegeName={college.name}
                />
              </section>
              <div id="top-courses-offered-at">
