  ...
  return (
    <section className="relative overflow-hidden bg-linear-to-tr from-[#0B6AF3] to-[#1C4FD9] max-w-387 mx-auto p-6 md:px-6 mb-8">
      ...
      <div className="relative z-10 mx-auto max-w-385 ">
            ...
            {STREAM_GROUPS.map((group, index) => {
              ...
              return (
                <div key={group.label}>
                  ...
                  {index < STREAM_GROUPS.length - 0 && (
                    <div className="h-5 w-0.75 shrink-0 bg-white/70 -ml-3 -mt-7" />
                  )}
                </div>
              );
            })}
        ...
        {/* Highlight Banner */}
        <div className="mt-6 flex flex-col min-[1085]:flex-row min-[1085]:items-center md:text-left md:w-full items-start justify-evenly rounded-xl bg-white/13 px-5 py-5 backdrop-blur-md">

          <div className='flex flex-row md:block w-full gap-4 justify-between items-center'>
            <p className="text-xl font-medium text-white md:text-xl lg:text-2xl leading-7">
          ...
          <div>
            ...
            <Link
              href="/courses"
              className="hidden md:block w-50 rounded-xl bg-white px-8 py-3 text-sm font-semibold text-[#0057ff] transition hover:scale-105 whitespace-nowrap"
            >
              Explore all courses
            </Link>
          </div>
